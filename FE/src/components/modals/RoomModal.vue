<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fade-in" @click.self="$emit('close')">
    <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full mx-4 animate-slide-in">
      <!-- Header -->
      <div class="bg-gradient-to-r from-purple-500 to-purple-600 text-white px-6 py-4 rounded-t-2xl">
        <h3 class="text-xl font-bold">{{ room ? 'Chỉnh sửa phòng' : 'Thêm phòng mới' }}</h3>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            class="input-field"
            placeholder="Phòng VIP sang trọng, view đẹp..."
          ></textarea>
        </div>
